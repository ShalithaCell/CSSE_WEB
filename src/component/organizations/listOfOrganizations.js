...
class ListOfOrganizations extends Component
{
	...
	{
		if (e.currentTarget.id === 'btnAddOrganization')
		{
			this.setState({
				...this.state,
            ...
            <Container fixed>
                <Card className={ classes.root }>
                    <CardHeader
								...
								className={ 'text-left' }
								action={
    <Button variant="outlined" color="primary" startIcon={ <AddCircle /> } id="btnAddOrganization" onClick={ this.onClickListner.bind(this) }>
        Add Organization
    </Button>
								}
							/>
        ...
        <Dialog open={ this.state.showNewUserDialog } aria-labelledby="form-dialog-title" fullWidth={ true } maxWidth={ 'md' }>
            ...
            <DialogContent>
                { <NewOrganization editable={ this.state.editable } userID={ this.state.editUserID } editRole={ this.state.editRole } dialogClose={ this.onClose }/> }
            </DialogContent>
        </Dialog>
...
export default connect(mapStateToProps, { updateUserList, removeUser })(withStyles(useStyles)(ListOfOrganizations));
